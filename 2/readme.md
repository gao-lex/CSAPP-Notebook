...
> 舍入方式：向偶数舍入、向零舍入、向下舍入、向上舍入

**向偶数舍入(round-to-even)** 也称为 **向最接近的值舍入(round-to-nearest)**：将数字向上或者向下舍入，使得结果的最低有效数字是偶数。
...
向偶数舍入的理由：避免统计误差： `50%` 向上舍入，`50%` 向下舍入

---

### 浮点运算

当浮点值 `x` 和 `y` 进行某个运算 `x⊙y` 时，计算将产生 `Round(x⊙y)` ,这是对实际运算的精确结果进行舍入后的结果。当参数中有一个是特殊值( `-0` , `-∞` , `NaN` )时， `IEEE` 标准定义了一些合理的规则。例如，定义 `1/-0` 将产生 `-∞` ，`1/+∞` 将产生 `+∞` 。

整数加法形成了 **阿贝尔群** 。实数加法也形成了阿贝尔群，但我们需要考虑舍入对这些属性的影响。我们将 `x+y` 定义为 `Round(x+y)` 。虽然 x 和 y 都是实数，由于溢出，该运算可能得到无穷值。这个运算是 **可交换** 的，即 `x+y = y+x` 。这个运算是 **不可结合的** 。

eg：利用单精度浮点值， `(3.14+1e10)-1e10 = 0.0` (3.14被舍入丢失)。 `3.14 + (1e10-1e10) = 3.14` (作为加法，大多数值在浮点加法下都有逆元，即 `x+(-x)=0` ,无穷和 `NaN` 是例外，因为对于任何 `x` ，都有 `Nan+x = NaN` ) 。

**浮点加法** 不具有结合性。浮点加法满足单调性属性(除了 `NaN` ，如果 `a>=b` ,那么对于任何 `a` , `b` 和 `x` 都有 `x+a>=x+b` ,无符号或补码的加法不具有这个实数和整数加法的属性)。

**浮点乘法** 可交换，不具有可结合性(由于舍入而失去精度)。浮点乘法在加法上不具有分配性。浮点乘法具有单调性(无符号或补码的乘法没有)。

---

### C语言中的浮点数

C语言标准不要求使用 `IEEE` 浮点，所以没有标准的方法来改变舍入方式（默认向偶数舍入）或者得到 `-0` 、 `+∞` ， `-∞` 或者 `NaN` 之类的特殊值。

```c
//GCC会定义程序常数INFINITY和NaN
...
### 练习53
 ...
 `int` 、 `float` 、 `double` 格式之间强制类型转换时，程序改变数值和位模式的原则如下：

* `int` → `float`：数字不会溢出，但可能被舍入
* `int` 或 `float` → `ouble` ：能精确保留数值
* `double` → `float` ：可能溢出成 `+∞` 或 `-∞` ，还有可能被舍入
* `float` 或 `double` → `int` ：值会向`0`舍入。
